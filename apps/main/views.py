...
class CategoryDetailView(generics.RetrieveUpdateDestroyAPIView):
    """API endpoint для перехода к конкретной категории"""
    queryset = Category.objects.all()
    serializer_class = CategorySerializer
    permission_classes = [permissions.IsAuthenticatedOrReadOnly]
    lookup_field = 'slug'
...
class PostListCreateView(generics.ListCreateAPIView):
    """
    API endpoint для постов с поддержкой закрепленных постов.
    ...
    def get_queryset(self):
        ...
        if not self.request.user.is_authenticated:
            ...
            # Если пользователь аторизован, показать его посты и все опубликованные посты
    ...
    def get_serializer_class(self): 
        # Если пришел POST запрос, верни сериализатор для обновления информации поста,
        # иначе верни сериализатор списка постов
...
def popular_posts(request):
    ...
    posts = Post.objects.with_subsscription_info.filter(
        status='published'
